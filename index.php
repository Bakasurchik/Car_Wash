...
$domainFilterOptions = array("Все домены");
...
}elseif (isset($_GET['geturl']))
{
    ...
    displayRuns($rs, "Запросы к URL: $url");
}elseif (isset($_GET['getcurl']))
{
    ...
    $rs = $xhprof_runs_impl->getRuns($criteria);
    include("../xhprof_lib/templates/emptyBody.phtml");
    displayRuns($rs, "Запросы с упрощенным URL: $url");
}elseif (isset($_GET['getruns']))
{
    ...
    $criteria['limit'] = "500";
    $criteria['where'] = "date('now', '-$days day') <= `timestamp`";
    $rs = $xhprof_runs_impl->getRuns($criteria);
    displayRuns($rs, "Запросы по максимальному значению $load_text");
}elseif(isset($_GET['hit']))
{
    ...
    echo "<h1 class=\"runTitle\">Самые посещаемые</h1>\n";
    echo "<table id=\"box-table-a\" class=\"tablesorter\" summary=\"Stats\"><thead><tr><th>URL</th><th>Посещений</th><th class=\"{sorter: 'numeric'}\">Суммарное время выполнения</th><th>Среднее время выполнения</th></tr></thead>";
    echo "<tbody>\n";
...
}else
{
    ...
    $criteria['limit'] = $last;
    $rs = $xhprof_runs_impl->getRuns($criteria);
    displayRuns($rs, "Последние $last запросов");
}
